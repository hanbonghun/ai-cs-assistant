...
> **측정 조건.** 임베딩은 실제 모델이 아니라 난이도별로 고정한 합성 벡터입니다 — 검색 로직을 바꿨을 때
> 원인을 분리하려면 임베딩이 결정론적이어야 하기 때문입니다. 따라서 이 표는 임베딩 품질이 아니라
> **게이트 · RRF · 한국어 전처리의 회귀**를 재는 값입니다. 실제 임베딩 기준의 품질 수치는 아직 없습니다.
...
평가 코퍼스는 정책 문서 6종(각 1문장)이고 임베딩은 난이도별 합성 벡터입니다. `"정답이 vector top-K 밖으로
밀려난 상황"` 같은 회귀 케이스는 임베딩을 통제할 수 있어야 재현되기 때문입니다. 정답 판정은 문서 제목
일치로만 하며, CSV 의 `expectedChunkKeywords` · `answerMustInclude` 컬럼은 아직 판정에 쓰지 않습니다.
...
처음에는 pgvector만 사용했습니다. 골든셋의 hard 케이스는 vector 임계값 아래에 놓이도록 설계해 두었고, `pg_trgm` 검색과 RRF를 붙였을 때 실제 한국어 키워드 매칭이 그중 얼마를 건져 올리는지를 봤습니다.

변경 후 hard Recall@3은 `0.000 → 0.143`, 전체 MRR은 `0.806 → 0.833`으로 올라갔고 NoMatchAccuracy는 1.0을 유지했습니다. 임베딩이 합성이므로 이 숫자는 검색 로직의 회귀 지표이지 임베딩 품질 지표가 아닙니다.
