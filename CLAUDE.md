...
- 커밋 메시지는 한국어로 작성한다.
- main 은 직접 push 가 막혀 있다. 브랜치 → PR → `gh pr merge --rebase` 로 넣는다.

---

## 문서 규칙

- 되돌리기 어려운 결정은 `docs/adr/` 에 ADR 로 남긴다 — 번호를 이어 붙이고 `docs/adr/README.md` 표도 갱신한다.
- 스펙·계획 문서는 git 추적 대상이 아니다 (`docs/superpowers/` 는 `.gitignore`). 의사결정 기록과 실행 계획은 수명이 다르다 — 왜 그렇게 정했는지는 코드가 바뀌어도 유효하지만, 어떤 순서로 만들라는 지시서는 실행이 끝나면 `src/` 와 중복되고 낡는다.
